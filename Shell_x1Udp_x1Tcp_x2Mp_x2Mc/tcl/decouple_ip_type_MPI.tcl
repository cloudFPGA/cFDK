...
set DecouplerType [list CONFIG.ALL_PARAMS {HAS_AXI_LITE 0 HAS_AXIS_CONTROL 0 INTF { ROLE {ID 0 SIGNALS { \
        MPE_ROLE_MPIif_out_mpi_call_TREADY {DIRECTION out} \
        ROLE_MPE_MPIif_in_mpi_call_TDATA {DIRECTION out WIDTH 8} \
        ROLE_MPE_MPIif_in_mpi_call_TVALID {DIRECTION out} \
        ROLE_MPE_MPIif_in_count_TDATA {DIRECTION out WIDTH 32} \
        ROLE_MPE_MPIif_in_count_TVALID {DIRECTION out} \
        MPE_ROLE_MPIif_out_count_TREADY {DIRECTION out} \
        ROLE_MPE_MPIif_in_rank_TDATA  {DIRECTION out WIDTH 32} \
        ROLE_MPE_MPIif_in_rank_TVALID {DIRECTION out} \
        MPE_ROLE_MPIif_out_rank_TREADY {DIRECTION out} \
        ROLE_MPE_MPI_data_TDATA {DIRECTION out WIDTH 8} \
        ROLE_MPE_MPI_data_TVALID {DIRECTION out} \
        ROLE_MPE_MPI_data_TKEEP {DIRECTION out} \
        ROLE_MPE_MPI_data_TLAST {DIRECTION out} \
        MPE_ROLE_MPI_data_TREADY {DIRECTION out} \
        EMIF_2B_Reg {DIRECTION out WIDTH 16 DECOUPLED_VALUE 0xadde} \
        Mem_Up0_Axis_RdCmd_tdata {DIRECTION out WIDTH 72} \
        Mem_Up0_Axis_RdCmd_tvalid {DIRECTION out} \
        Mem_Up0_Axis_RdSts_tready {DIRECTION out} \
        Mem_Up0_Axis_Read_tready {DIRECTION out} \
        Mem_Up0_Axis_WrCmd_tdata {DIRECTION out WIDTH 72} \
