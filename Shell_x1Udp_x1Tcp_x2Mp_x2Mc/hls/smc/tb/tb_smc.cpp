...
int main(){
  ...
  smc_main(sys_reset, &MMIO_in, &MMIO, HWICAP, 0b1, &decoupActive, &softReset, xmem, mpeCtrl, &nodeRank, &clusterSize);
  printf("%#010x\n", (int) MMIO);
...
//Test Displays
  ...
  smc_main(sys_reset, &MMIO_in, &MMIO, HWICAP, 0b0, &decoupActive, &softReset, xmem, mpeCtrl, &nodeRank, &clusterSize);
  printf("%#010x\n", (int) MMIO);
...
//Test Counter & Xmem
  ...
  cnt = 2;
  //initBuffer((ap_uint<4>) cnt, xmem, mpeCtrl, &nodeRank, &clusterSize);
  smc_main(sys_reset, &MMIO_in, &MMIO, HWICAP, 0b0, &decoupActive, &softReset, xmem, mpeCtrl, &nodeRank, &clusterSize);
...
/*  copyBufferToXmem(&httpBuffer[256],xmem, mpeCtrl, &nodeRank, &clusterSize);
  //printBuffer32(xmem, "Xmem:",2);
  smc_main(sys_reset, &MMIO_in, &MMIO, HWICAP, 0b0, &decoupActive, &softReset, xmem, mpeCtrl, &nodeRank, &clusterSize);
...
/*  printf("===== MPE =====\n");
  ...
  //RST 
  MMIO_in = 0x3 << DSEL_SHIFT | (1 << RST_SHIFT);
  smc_main(0b1, &MMIO_in, &MMIO, HWICAP, 0b0, &decoupActive, &softReset, xmem, mpeCtrl, &nodeRank, &clusterSize);
  smc_main(sys_reset, &MMIO_in, &MMIO, HWICAP, 0b0, &decoupActive, &softReset, xmem, mpeCtrl, &nodeRank, &clusterSize);
  succeded &= checkResult(MMIO, 0x3f49444C);
